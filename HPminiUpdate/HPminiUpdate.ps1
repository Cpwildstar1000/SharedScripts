...
if ($Confirmation -eq "Y") {
    ...
    try {
        ...
        if (Invoke-Command -ComputerName $FullComputerName -ScriptBlock { Test-Path C:\DriverInstallFiles }) {
            ...
            foreach ($Driver in $Drivers) {
                ...
                try {
                    ...
                    "Copied $File to $FullComputerName" | Tee-Object $LogFile -Append
                }
                catch {
                    "Failed to copy $File to $FullComputerName : $($_.Exception.Message)" | Tee-Object $LogFile -Append | Write-Host -ForegroundColor Red
        ...
        foreach ($Driver in $Drivers) {
            ...
            elseif ($result.Status -eq 'TimedOut') {
                "$File installer TIMED OUT after configured timeout" | Tee-Object $LogFile -Append | Write-Host -ForegroundColor Red
            }
            elseif ($result.Status -eq 'Failed') {
                "$File installer failed to start: $($result.Message)" | Tee-Object $LogFile -Append | Write-Host -ForegroundColor Red
            }
            else {
